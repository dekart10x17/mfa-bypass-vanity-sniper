...
try {
    ...
    console.error("config de hata var ", error);
    process.exit(1);
}
...
console.log("dekart1017 mfa bypass");
...
class SessionManager {
    ...
    createSession() {
        ...
        this.session.on('connect', () => {
            console.log(colors.green("HTTP/2 Connect"));
            this.isConnecting = false;
        });
        this.session.on('close', () => {
            console.log(colors.yellow("HTTP/2 Closed"));
            this.isConnecting = false;
            setTimeout(() => this.createSession(), 5000);
        });
    }
...
async function refreshMfaToken() {
    try {
        ...
        if (data.code === 60003) {
            ...
            console.log(colors.yellow("Mfa Bypass -dekart1017"));
        ...
        embeds: [
            {
                ...
                fields: [
                    { name: 'FAST', value: `\`${find}\``, inline: true },
                    { name: 'FAST', value: `\`${elapsedTime}ms\``, inline: true },
                ],
                footer: {
                    text: `${new Date().toLocaleString('tr-TR', { hour12: false })}`,
                    icon_url: 'buraya foto',
                },
            ...
            switch (payload.op) {
                ...
                case 0:
                    ...
                    if (type === "GUILD_UPDATE") {
                        ...
                        console.log(colors.green("Dekart 1017"));
                    }
                    break;
                case 7:
...
async function initialize() {
    try {
        ...
        console.log("Refresh Mfa Token -dekart1017");
        connectWebSocket();
